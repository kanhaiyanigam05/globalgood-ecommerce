        ...
        <div class="row m-1">
            <div class="col-12">
                ...
                <ul class="app-line-breadcrumbs mb-3">
                    ...
                    <li>
                        <a class="f-s-14 f-w-500" href="{{ route('admin.products.edit', Crypt::encryptString($product->id)) }}">
                            <span><i class="ph-duotone ph-folder f-s-16"></i> {{ $product->title }}</span>
                        </a>
                    </li>
                    <li class="active">
                        <a class="f-s-14 f-w-500" href="javascript:void(0)">{{ $variant->attributes->map(fn($a) => $a->pivot->value)->implode(' / ') ?: 'Default Variant' }}</a>
                    </li>
                </ul>
            </div>
        </div>
        ...
        <div class="row g-0">
            ...
            <div class="col-md-9 col-lg-10 p-4 bg-light">
                <div class="mx-auto" style="max-width: 900px;">
                    <x-forms.form varient="reactive" :action="route('admin.products.variants.update', [$encryptedProductId, \Crypt::encryptString($variant->id)])" method="PUT">
                                ...
                                @foreach ($attributes as $index => $attribute)
                                        <div class="mb-3">
                                            ...
                                            <x-forms.select 
                                                ...
                                                :options="$attribute->values->pluck('value', 'value')->toArray()"
                                                :value="$variantAttributesMap[$attribute->id] ?? ''"
                                                placeholder="Select {{ $attribute->name }}"
                                                :error="$errors->first('attributes.'.$index.'.value')"
                                                disabled
                                            />
                                        </div>
                                @endforeach
                        ...
                        <!-- Pricing Section -->
                        <div class="card mb-4 shadow-sm @if($errors->has('price') || $errors->has('compare_at_price')) border border-danger @else border-0 @endif">
                            ...
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-md-6 mb-3">
                                        <x-forms.input type="number" name="price" label="Price" step="0.01" :value="old('price', $variant->price_formatted)" :error="$errors->first('price')" required />
                                        <small class="text-muted"><i class="ph ph-info me-1"></i>Customer-facing price</small>
                                    </div>
                                    <div class="col-md-6 mb-3">
                                        <x-forms.input type="number" name="compare_at_price" label="Compare-At Price" step="0.01" :value="old('compare_at_price', $variant->compare_at_price_formatted)" :error="$errors->first('compare_at_price')" />
                                ...
                                @if(old('compare_at_price', $variant->compare_at_price_formatted))
                                    ...
                                    @if($savings > 0)
                                        <div class="alert alert-success mt-3 mb-0 border-0 shadow-sm">
                                            ...
                                            <strong>Customers save {{ number_format($savings, 0) }}%</strong> (${{ number_format($comparePrice - $price, 2) }})
                                        </div>
                                    @endif
                                @endif
                            </div>
                        ...
                        <!-- Inventory Section -->
                        <div class="card mb-4 shadow-sm @if($errors->has('quantity') || $errors->has('sku')) border border-danger @else border-0 @endif">
                                    ...
                                    <table class="table mb-0 align-middle">
                                        ...
                                        <tbody>
                                            <tr>
                                                ...
                                                <td>
                                                    <x-forms.input label="Quantity" type="number" name="quantity" :value="old('quantity', $variant->quantity)" :error="$errors->first('quantity')" required style="width: 120px;" min="0" />
                                                </td>
                                ...
                                <div class="p-4 border-top bg-light-subtle">
                                    <div class="row">
                                        <div class="col-md-6 mb-3 mb-md-0">
                                            <x-forms.input name="sku" label="SKU (Stock Keeping Unit)" :value="old('sku', $variant->sku)" :error="$errors->first('sku')" placeholder="e.g. WH-01-L" />
                                        </div>
                                        {{-- <div class="col-md-6">
                        ...
                        <div class="d-flex justify-content-between align-items-center mb-5">
                            <a href="{{ route('admin.products.edit', $encryptedProductId) }}" class="btn btn-outline-secondary">
