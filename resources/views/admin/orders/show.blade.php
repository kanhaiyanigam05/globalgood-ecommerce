...
@section('content')
    <div class="container-fluid">
        ...
        <div class="row m-1 align-items-center mb-4 order-header-section">
            <div class="col-md-6">
                <div class="d-flex align-items-center gap-2 flex-wrap">
                    <a href="{{ route('admin.orders.index') }}" class="btn btn-outline-secondary btn-sm back-btn" title="Back to orders">
                        <i class="ph ph-arrow-left"></i>
                    </a>
                    <h4 class="main-title mb-0 order-number-title">{{ $order->order_number }}</h4>
                    <span class="badge status-badge badge-{{ $order->order_status === 'open' ? 'primary' : ($order->order_status === 'canceled' ? 'danger' : 'secondary') }}">
                        <i class="ph ph-circle me-1"></i>{{ ucfirst($order->order_status ?? 'Open') }}
                    </span>
                    <span class="badge status-badge badge-{{ $order->payment_status === 'paid' ? 'success' : ($order->payment_status === 'pending' ? 'warning' : ($order->payment_status === 'refunded' ? 'danger' : 'secondary')) }}">
                        <i class="ph ph-credit-card me-1"></i>{{ ucfirst($order->payment_status ?? 'Pending') }}
                    </span>
                    <span class="badge status-badge badge-{{ $order->fulfillment_status === 'fulfilled' ? 'success' : 'info' }}">
            ...
            <div class="col-md-6 text-md-end mt-3 mt-md-0">
                <div class="d-flex gap-2 justify-content-md-end flex-wrap">
                    ...
                    <a href="{{ route('admin.orders.edit', $order->id) }}" class="btn btn-outline-primary btn-sm action-btn">
        ...
        <div class="row">
            <div class="col-lg-8">
                <div class="card status-alert-card border-0 mb-4 {{ $order->order_status === 'canceled' ? 'alert-danger' : ($order->order_status === 'archived' ? 'alert-secondary' : ($order->fulfillment_status === 'fulfilled' ? 'alert-success' : 'alert-warning')) }}">
                    <div class="card-body py-3 px-4">
                        <div class="d-flex align-items-center gap-3">
                            @if($order->order_status === 'canceled')
                                <div class="status-icon-wrapper warning" style="color: #dc3545; background: rgba(220, 53, 69, 0.15);">
                                    <i class="ph ph-x-circle"></i>
                                </div>
                                <div class="flex-grow-1">
                                    <span class="fw-semibold d-block mb-1 status-title">Order Canceled</span>
                                    <div class="small text-dark-emphasis">
                                        This order was canceled. You can view the details or <a href="{{ route('admin.orders.create') }}" class="text-decoration-none fw-medium link-primary">create a new order</a>.
                                    </div>
                                </div>
                            @elseif($order->order_status === 'archived')
                                <div class="status-icon-wrapper" style="color: #6c757d; background: rgba(108, 117, 125, 0.15);">
                            ...
                            @elseif($order->fulfillment_status === 'fulfilled')
                                ...
                                <div class="flex-grow-1">
                                    ...
                                    <div class="small text-dark-emphasis">
                                        Order created on {{ $order->created_at->format('M j, Y, g:i A') }}. You can view the order or <a href="{{ route('admin.orders.create') }}" class="text-decoration-none fw-medium link-primary">create a new order</a>.
                                    </div>
                                </div>
                            @else
                ...
                <div class="card modern-card mb-4">
                    <div class="card-header bg-gradient-light d-flex justify-content-between align-items-center">
                        <div class="d-flex align-items-center gap-2">
                            <span class="badge modern-badge badge-{{ $order->fulfillment_status === 'fulfilled' ? 'success' : 'secondary' }}">
                                <i class="ph ph-package me-1"></i>{{ ucfirst($order->fulfillment_status ?? 'Unfulfilled') }}
                            </span>
                                            ...
                                            <div class="d-flex align-items-center gap-3">
                                                <div class="product-image-wrapper">
                                                    @if($item->product && $item->product->images->count() > 0)
                                                        <img src="{{ route('file.path', ['path' => $item->product->images->first()->file, 'w' => 60, 'h' => 60]) }}" alt="{{ $item->name }}" class="product-image">
                                                    @else
                ...
                <div class="card modern-card mb-4">
                    <div class="card-header bg-gradient-light">
                        <div class="d-flex align-items-center gap-2">
                            <span class="badge modern-badge badge-{{ $order->payment_status === 'paid' ? 'success' : 'warning' }}">
                                <i class="ph ph-credit-card me-1"></i>{{ ucfirst($order->payment_status ?? 'Pending') }}
                            </span>
                    ...
                    <div class="card-body px-4 py-4">
                        <div class="payment-line-item">
                            <div class="d-flex justify-content-between align-items-center">
                                ...
                                <div class="d-flex align-items-center gap-3">
                                    <span class="line-meta">{{ $order->items->count() }} {{ Str::plural('item', $order->items->count()) }}</span>
                                ...
                                <div class="timeline-content">
                                    <div class="d-flex justify-content-between align-items-start flex-wrap gap-2">
                                        <div>
                                            ...
                                            <div class="timeline-detail">A ₹{{ $order->formatted_total }} INR payment was processed.</div>
                                        </div>
                        ...
                        @if($order->customer)
                            ...
                            <div class="customer-section">
                                ...
                                <div class="contact-item">
                                    <i class="ph ph-envelope me-2 text-primary"></i>{{ $order->email ?: 'No email provided' }}
                                </div>
                                <div class="contact-item">
                            ...
                            <div class="customer-section">
                                ...
                                @if($order->shipping_address)
                                    <div class="address-line">{{ $order->shipping_address['first_name'] ?? '' }} {{ $order->shipping_address['last_name'] ?? '' }}</div>
                                    ...
                                    @endif
                                    <div class="address-line">{{ $order->shipping_address['zip'] ?? '' }} {{ $order->shipping_address['city'] ?? '' }} {{ $order->shipping_address['province'] ?? '' }}, {{ $order->shipping_address['country'] ?? '' }}</div>
                                    <div class="contact-item mb-2">
                                        <i class="ph ph-phone me-2 text-primary"></i>{{ $order->shipping_address['phone'] ?? $order->shipping_address['tel'] ?? '' }}
                                    </div>
                                @else
                                ...
                                @if($order->shipping_address)
                                    @php
                                        ...
                                        $query = urlencode(implode(', ', array_filter([
                            ...
                            <div class="customer-section mb-0">
                                ...
                                @if($order->billing_address)
                                    ...
                                    @else
                                        <div class="address-line">{{ $order->billing_address['first_name'] ?? '' }} {{ $order->billing_address['last_name'] ?? '' }}</div>
                                        <div class="address-line">{{ $order->billing_address['address1'] ?? '' }}</div>
                                        <div class="address-line">{{ $order->billing_address['zip'] ?? '' }} {{ $order->billing_address['city'] ?? '' }} {{ $order->billing_address['province'] ?? '' }}, {{ $order->billing_address['country'] ?? '' }}</div>
                                    @endif
                                @else
                                    <div class="address-line"><em>Same as shipping address</em></div>
                                @endif
