...
<div class="container-fluid py-4" style="max-width: 1100px;">
    <div class="d-flex align-items-center mb-4">
        ...
        <h4 class="mb-0 f-w-700">Edit discount</h4>
    </div>
                ...
                <div class="discount-card">
                    ...
                    <div class="discount-card-body">
                        ...
                        <div id="codeSection">
                            ...
                            <input type="text" name="code" id="discountCode" class="form-control" placeholder="e.g. SUMMER2024" value="{{ $discount->code }}">
                            <p class="text-muted f-s-12 mt-2 mb-0">Customers must enter this code at checkout.</p>
                        </div>

                        <div id="titleSection" class="d-none">
                            <label class="form-label">Title</label>
                            <input type="text" name="title" id="discountTitle" class="form-control" value="{{ $discount->title }}">
                        ...
                        <div class="row mb-4">
                            ...
                            <div class="col-md-7">
                                <div class="input-group">
                                    <input type="number" name="value" class="form-control" placeholder="0" value="{{ $discount->value }}">
                        ...
                        <div class="form-check mb-2">
                            ...
                            <div class="ms-4 mt-2 mb-3">
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" name="apply_on_pos" id="pos_pro" {{ $discount->apply_on_pos ? 'checked' : '' }}>
                    ...
                    <div class="discount-card-body">
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="date" name="starts_at_date" class="form-control" required value="{{ $discount->starts_at ? $discount->starts_at->format('Y-m-d') : date('Y-m-d') }}">
                            </div>
                            <div class="col-md-6 mb-3">
                                <label class="form-label">Start time (IST)</label>
                                <input type="time" name="starts_at_time" class="form-control" value="{{ $discount->starts_at ? $discount->starts_at->format('H:i') : date('H:i') }}">
                            </div>
                        </div>
                        <div class="form-check mb-3">
                            <input class="form-check-input" type="checkbox" id="setEndDate" {{ $discount->ends_at ? 'checked' : '' }}>
                            <label class="form-check-label" for="setEndDate">Set end date</label>
                        </div>
                        <div id="endDateSection" class="row d-none">
                            <div class="col-md-6 mb-3">
                                <label class="form-label">End date</label>
                                <input type="date" name="ends_at_date" class="form-control" value="{{ $discount->ends_at ? $discount->ends_at->format('Y-m-d') : '' }}">
                            </div>
                            <div class="col-md-6 mb-3">
                                <label class="form-label">End time</label>
                                <input type="time" name="ends_at_time" class="form-control" value="{{ $discount->ends_at ? $discount->ends_at->format('H:i') : '23:59' }}">
                            </div>
                        </div>
                    </div>
                </div>
...
<script>
    $(document).ready(function() {
        ...
        // EDIT MODE: Load existing selections on page load
        @if(isset($discount) && $discount->exists)
            // Load existing targets (products, variants, collections)
            const existingTargets = @json($discount->targets);
            existingTargets.forEach(target => {
                let name = '';
                let icon = 'tag';
                
                if (target.target_type === 'product') {
                    const product = products.find(p => p.id === target.target_id);
                    if (product) {
                        name = product.title;
                        icon = 'tag';
                        // Set applies to dropdown
                        $('#appliesTo').val('products').trigger('change');
                    }
                } else if (target.target_type === 'variant') {
                    products.forEach(product => {
                        const variant = product.variants?.find(v => v.id === target.target_id);
                        if (variant) {
                            const attrs = variant.attributes || [];
                            const vName = attrs.map(a => a.pivot?.value || '').filter(v => v).join(' / ') || 'Default';
                            name = `${product.title} - ${vName}`;
                            icon = 'tag';
                            $('#appliesTo').val('products').trigger('change');
                        }
                    });
                } else if (target.target_type === 'collection') {
                    const collection = collections.find(c => c.id === target.target_id);
                    if (collection) {
                        name = collection.title;
                        icon = 'folders';
                        $('#appliesTo').val('collections').trigger('change');
                    }
                }
                
                if (name) {
                    const uniqueId = `selected-${target.target_type}-${target.target_id}`;
                    $('#selectedItems').append(`
                        <div class="d-flex align-items-center justify-content-between p-2 mb-2 bg-light rounded" id="${uniqueId}">
                            <div class="d-flex align-items-center">
                                <i class="ph ph-${icon} me-2"></i>
                                <span class="f-s-13">${name}</span>
                                <input type="hidden" name="targets[]" value="${target.target_id}">
                                <input type="hidden" name="target_types[]" value="${target.target_type}">
                            </div>
                            <button type="button" class="btn btn-link link-danger p-0 ms-2" onclick="$(this).closest('.bg-light').remove()">
                                <i class="ph ph-x"></i>
                            </button>
            ...
            @if($discount->customer_selection === 'specific')
                $('#customerSelectionArea').removeClass('d-none');
                const existingCustomers = @json($discount->customers);
                existingCustomers.forEach(customer => {
                    const name = customer.full_name || `${customer.first_name} ${customer.last_name}`;
                    const uniqueId = `selected-customer-${customer.id}`;
                    $('#selectedCustomers').append(`
                        <div class="d-flex align-items-center justify-content-between p-2 mb-2 bg-light rounded" id="${uniqueId}">
                            <div class="d-flex align-items-center">
            ...
            // Trigger method change to show correct section
            const currentMethod = '{{ $discount->method }}';
            if (currentMethod === 'automatic') {
                $('#codeSection').addClass('d-none');
                $('#titleSection').removeClass('d-none');
            }

            // Trigger end date section if exists
            @if($discount->ends_at)
